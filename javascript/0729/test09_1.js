...
function regExCheck() {
  ...
  const regEx9 = /[^0-9가-힣]/g;  // 숫자,한글만 포함?
  const regEx10 = /[^a-zA-Z0-9_-]/g;  // 영문 대/소문자, 숫자, _, - 만 포함?
  const regEx21 = /\./g; // . 을 포함??
  const regEx22 = /\d/g; // 숫자를 포함??
  const regEx23 = /\D/g; // 숫자가 아닌 다른값 포함??
  const regEx24 = /\w/g; // 영문/숫자/_ 값을 포함??
  const regEx25 = /\s/g; // 영문/숫자/_ 값을 포함??
  ...
  // if(regEx10.test(content)) alert("본문에 영문, 숫자, _, - 가 아닌 문자가 있습니다.");
  // if(!regEx21.test(content)) alert("본문에 .이 아닌 문자가 있습니다.");
  // if(!regEx22.test(content)) alert("본문에 숫자가 아닌 문자가 있습니다.");
  // if(!regEx23.test(content)) alert("본문에 숫자가 아닌 값이 있습니다.");
  if(regEx25.test(content)) alert("본문에 공백을 포함하고 있습니다.");
